...
Item {
    ...
    Item {
            ...
            function startAsyncExtraction(coverUrl) {
                ...
                currentTask = Qt.createQmlObject(`
                    import QtQuick 2.15

                    Item {
                        property url coverUrl: "${coverUrl}"
                        ...
                        Timer {
                            ...
                            running: true
                            onTriggered: {
                                console.log("开始异步颜色提取:", parent.coverUrl)
                                parent.startExtraction()
                            }
                        }
                        ...
                        function startExtraction() {
                            ...
                            var checkStatus = function() {
                                ...
                                if (tempImage.status === Image.Ready) {
                                    console.log("异步: 图片加载完成")
                                    tempImage.grabToImage(function(result) {
                                        if (result && result.image) {
                                            try {
                                                console.log("图片抓取成功，调用C++颜色提取")
                                                var colors = p_imageColor.getMainColors(result.image)
                                                console.log("异步: 颜色提取完成")
                                                parent.extractionComplete(colors)
                                            } catch (error) {
                                                console.error("异步: 颜色提取失败:", error)
                                                parent.extractionFailed()
                                            }
                                        } else {
                                            parent.extractionFailed()
                                        }
                ...
                currentTask.extractionComplete.connect(function(colors) {
                ...
                currentTask.extractionFailed.connect(function() {
